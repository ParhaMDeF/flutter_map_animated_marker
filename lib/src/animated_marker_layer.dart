import 'package:dart_exporter_annotation/dart_exporter_annotation.dart';
import 'package:flutter/material.dart';
import 'package:flutter_map/flutter_map.dart';
import 'package:flutter_map_animated_marker/src/animated_marker_layer_options.dart';
import 'package:latlong2/latlong.dart';

@Export()
class AnimatedMarkerLayer<T> extends ImplicitlyAnimatedWidget {
  final AnimatedMarkerLayerOptions<T> options;

  AnimatedMarkerLayer({
    Key? key,
    required this.options,
  }) : super(
          key: key,
          duration: options.duration,
          curve: options.curve,
        );

  @override
  AnimatedWidgetBaseState createState() => _AnimatedMarkerLayerState();
}

class _AnimatedMarkerLayerState
    extends AnimatedWidgetBaseState<AnimatedMarkerLayer>
    with AutomaticKeepAliveClientMixin {
  Tween<double>? _latitude;
  Tween<double>? _longitude;

  Marker get marker => widget.options.marker;

  double get latitude =>
      _latitude?.evaluate(animation) ?? marker.point.latitude;
  double get longitude =>
      _longitude?.evaluate(animation) ?? marker.point.longitude;

  @override
  void forEachTween(TweenVisitor<dynamic> visitor) {
    _latitude = visitor(
      _latitude,
      widget.options.marker.point.latitude,
      (dynamic value) => Tween<double>(begin: value as double),
    ) as Tween<double>?;

    _longitude = visitor(
      _longitude,
      widget.options.marker.point.longitude,
      (dynamic value) => Tween<double>(begin: value as double),
    ) as Tween<double>?;
  }

  @override
  void didUpdateWidget(covariant AnimatedMarkerLayer oldWidget) {
    super.didUpdateWidget(oldWidget);
    setState(() {});
  }

  @override
  Widget build(BuildContext context) {
    super.build(context);

    // v8 camera API
    final map = MapCamera.of(context);

    // Anchor math (same as before)
    final width = marker.width;
    final height = marker.height;
    final left = 0.5 * width * (((marker.alignment)?.x ?? 0) + 1);
    final top = 0.5 * height * (((marker.alignment)?.y ?? 0) + 1);
    final right = width - left;
    final bottom = height - top;

    // Compute on-screen position in layer space using v8 helper
    // (accounts for rotation & pixel origin internally)
    final pos = map.getOffsetFromOrigin(
        LatLng(latitude, longitude)); // :contentReference[oaicite:3]{index=3}

    // Quick visibility culling: does the marker's rect overlap the map's rect?
    final markerRect =
        Rect.fromLTWH(pos.dx - right, pos.dy - bottom, width, height);
    final mapRect = Rect.fromLTWH(
        0, 0, map.size.width, map.size.height); // rotated bounding box
    if (!markerRect.overlaps(mapRect)) {
      return const SizedBox();
    }

    // Optional counter-rotation (same behavior as MarkerLayer 'rotate')
    final markerWidget = (marker.rotate ?? widget.options.rotate ?? false)
        ? Transform.rotate(
            angle: -map.rotationRad, // v8 camera prop
            origin: widget.options.rotateOrigin,
            alignment: widget.options.rotateAlignment,
            child: marker.child,
          )
        : marker.child;

    // Keep this so the layer tracks camera movement/rotation properly in v8
    return MobileLayerTransformer(
      // :contentReference[oaicite:4]{index=4}
      child: Stack(
        children: [
          Positioned(
            key: marker.key,
            width: marker.width,
            height: marker.height,
            left: pos.dx - right,
            top: pos.dy - bottom,
            child: markerWidget,
          ),
        ],
      ),
    );
  }

  @override
  bool get wantKeepAlive => true;
}
